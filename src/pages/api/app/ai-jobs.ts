...
export const POST: APIRoute = async (c) => {
  try {
    ...
    // A board search result has a synthetic `board:<external_id>` id and no row yet, so this
    // materializes it rather than passing that id to a uuid column.
    const jobRow = await resolveJobRow(db, user.id, { ...b, job_profile_id: id }, { status: "delegated" });
    const jobId = jobRow.id;
    if (jobRow.status !== "delegated") {
